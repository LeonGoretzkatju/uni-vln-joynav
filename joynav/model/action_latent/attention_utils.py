...
class Attention(nn.Module):
    ...
    def get_processor(self, return_deprecated_lora: bool = False) -> "AttnProcessor2_0":
        r"""
...
class FeedForward(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecation_message = "The `scale` argument is deprecated and will be ignored. Please remove it, as passing it will raise an error in the future. `scale` should directly be passed while calling the underlying pipeline component i.e., via `cross_attention_kwargs`."
            print(f"scale , 1.0.0, {deprecation_message}")
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        if positional_embeddings == "sinusoidal":
            raise NotImplementedError("Sinusoidal positional encoding is not yet implemented.")
        else:
            self.pos_embed = None
        ...
        if norm_type == "ada_norm":
            raise NotImplementedError("Adversarial normalization not yet implemented.")
        else:
...
class SelfAttentionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.attention_head_dim = attention_head_dim
        self.inner_dim = num_attention_heads * self.attention_head_dim
