...
# LoRA training writes a merged standalone checkpoint under <output_dir>/merged. Point
# MODEL_PATH there (NOT the bare adapter dir) — the evaluator loads with from_pretrained and
# cannot consume a LoRA adapter. Keep OMNI_HISTORY_IMAGES / MAX_PIXELS identical to training.
MODEL_PATH=${MODEL_PATH:-outputs/qwen3_5_4b_sf_omega_omni_lora/merged}
OUTPUT_PATH=${OUTPUT_PATH:-results/r2r/val_unseen/qwen3_5_4b_sf_omega_omni_lora}
# Single front camera, matching training (sparse front history + one current front frame).
