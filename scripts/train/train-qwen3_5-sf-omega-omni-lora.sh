...
# --- LoRA configuration -------------------------------------------------------
# LoRA adapts the Qwen3.5 LLM (full-attention q/k/v/o + every MLP, incl. MoE experts by
# suffix); the vision tower stays frozen. The freshly-initialized omni heads + SF projector +
# merger train fully via modules_to_save (train_qwen.py auto-adds action_head /
# spatial_forcing_projector / arrive_predictor / query_multihead_attn, and merger when
# tune_mm_mlp=True). The bare `query_action` parameter is unfrozen in train_qwen.py.
# After training, the adapter is merged into a full standalone checkpoint under
# ${output_dir}/merged (the Habitat eval path loads with from_pretrained and cannot read a
# bare adapter) — point the eval script's MODEL_PATH at that merged dir.
lora_r=${LORA_R:-16}
lora_alpha=${LORA_ALPHA:-32}
lora_dropout=${LORA_DROPOUT:-0.05}
lora_target_modules=${LORA_TARGET_MODULES:-q_proj,k_proj,v_proj,o_proj,gate_proj,up_proj,down_proj}
lora_modules_to_save=${LORA_MODULES_TO_SAVE:-action_head,arrive_predictor,query_multihead_attn,spatial_forcing_projector}
...
args="
    ...
    --lora_merge_and_save ${LORA_MERGE_AND_SAVE:-True} \
    --sf_enabled True \
    --sf_alpha ${sf_alpha} \
