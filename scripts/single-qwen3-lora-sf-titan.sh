...
run_name=${RUN_NAME:-"qwen3_vl_2b_lora_sf_titan"}
output_dir=${OUTPUT_DIR:-./outputs/${run_name}/stage1-r2r+rxr-lora_r${lora_r}-sf_alpha_${sf_alpha}-layers_${sf_align_layers}-lr_${lr}-mm_lr_${mm_projector_lr}-batch_size_${batch_size}-grad_accum_steps_${grad_accum_steps}-epochs_${num_train_epochs}-max_pixels_${max_pixels}}
if [ "${max_steps}" != "-1" ]; then
